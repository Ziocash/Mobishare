  ...
  form.addEventListener("submit", function (event) {
    ...
    fetch("?handler=ReserveVehicle", {
      ...
      .then((response) =>
        response.ok
          ? response.text()
          : Promise.reject("Errore nella prenotazione")
      )
      .then((data) => {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  hub.on("ReceiveVehiclePositionUpdate", (vehicle) => {
    ...
    if (vehicleMarkers[id]) {
      ...
      if (
        currentPos.lat() !== newPosition.lat ||
        currentPos.lng() !== newPosition.lng
      ) {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      function (position) {
        ...
        // Salva la mappa in una variabile globale
...
async function startTimer() {
  ...
  timer_conn = new signalR.HubConnectionBuilder().withUrl("/timerHub").build();
...
function freeVehicle(id) {
  fetch("?handler=FreeVehicle", {
    ...
    .then((response) =>
      response.ok ? response.text() : Promise.reject("Errore nella liberazione")
    )
    .then((data) => {
